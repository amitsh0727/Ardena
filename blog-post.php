...
if (file_exists($blogsFile) && $slug) {
    $data = json_decode(file_get_contents($blogsFile), true);
    $blogs = $data['blogs'] ?? [];
    foreach ($blogs as $b) {
        if ($b['slug'] === $slug && ($b['status'] ?? 'published') === 'published') {
            $blog = $b;
            break;
        }
    }
...
$pageTitle = $blog['meta_title'] ?? $blog['title'] . ' | Ardena';
...
<head>
    ...
    <meta property="og:image" content="<?php echo htmlspecialchars($blog['featured_image']); ?>">
            ...
            <div id="swupMain" class="mil-main-transition">
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-10">
                                <div class="mil-labels mil-up mil-mb-30">
                                    ...
                                    <div class="mil-label mil-upper"><?php echo date('F d, Y', strtotime($blog['published_date'])); ?></div>
                                </div>
                                <h1 class="mil-up mil-mb-30"><?php echo htmlspecialchars($blog['title']); ?></h1>
                                <p class="mil-up mil-text-lg"><?php echo htmlspecialchars($blog['excerpt']); ?></p>
                                <div class="mil-up mil-mt-30">
                                    <span class="mil-text-sm">By <?php echo htmlspecialchars($blog['author']); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-10">
                                <div class="mil-image-frame mil-up">
                                    <img src="<?php echo htmlspecialchars($blog['featured_image']); ?>" alt="<?php echo htmlspecialchars($blog['title']); ?>" class="mil-scale" data-value-1="1" data-value-2="1.1">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section>
                    <div class="container mil-p-120-120">
                        <div class="row justify-content-center">
                            <div class="col-lg-8">
                                <div class="mil-text mil-up">
                                    <?php echo $blog['content']; ?>
                                </div>
                                
                                <?php if (!empty($blog['tags'])): ?>
                                <div class="mil-tags mil-up mil-mt-60">
                                    <strong>Tags: </strong>
                                    <?php foreach ($blog['tags'] as $tag): ?>
                                    <span class="mil-tag"><?php echo htmlspecialchars($tag); ?></span>
                                    <?php endforeach; ?>
                                </div>
                                <?php endif; ?>
